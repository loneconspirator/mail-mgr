...
**Skip this entire step if the caller passed `--skip-tests`** (the `/validate` orchestrator does this so it can run the whole test suite once in parallel instead of one-file-at-a-time per validator). When skipping, note in the report that the integration test run was deferred to the orchestrator's full-suite run.
