...
All heavy lifting is handled by shell scripts in this directory. Do NOT freestyle shell commands for starting, stopping, or waiting — use these scripts.

| Script | Purpose |
|--------|---------|
| `start.sh` | Start GreenMail, seed data, launch app, wait for readiness |
| `stop.sh` | Kill app, stop GreenMail, clean up `/tmp/mail-mgr-dev` |
| `wait-for-port.sh <port> <label> [timeout]` | Wait for a TCP port to accept connections |
...
1. Run the stop script: `.claude/skills/dev-env/stop.sh`
2. Run the start script: `.claude/skills/dev-env/start.sh`
...
2. Run the check script:
   ```bash
   DATA_PATH=/tmp/mail-mgr-dev npx tsx .claude/skills/dev-env/check.ts
   ```
...
- The seed data is defined in `.claude/skills/dev-env/seed-data.yml` — edit that file to change what gets populated
- The seed script is at `.claude/skills/dev-env/seed.ts`
- The check script is at `.claude/skills/dev-env/check.ts`
- GreenMail credentials: user=`user`, pass=`pass`, host=`localhost`

## Seed Data File

The seed data YAML file (`.claude/skills/dev-env/seed-data.yml`) contains:
