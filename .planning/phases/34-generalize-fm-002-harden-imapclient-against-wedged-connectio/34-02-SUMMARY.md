...
- **Task 2 — Inner-work timeouts tuned per op-class + flow.logout bounded + idleTimeout default lowered + 22 fixtures synced.** moveMessage / fetchNewMessages / appendMessage / searchByHeader pass WRITE_TIMEOUT_MS; fetchAllMessages passes BULK_FETCH_TIMEOUT_MS (long-running bulk path); deleteMessage / createMailbox / renameFolder pass LIST_TIMEOUT_MS; getSpecialUseFolder / fetchMessagesRaw / listMailboxes pass LIST_TIMEOUT_MS. `disconnect()` now bounds `flow.logout()` with 5_000ms via withTimeout. `imapConfigSchema.idleTimeout` default lowered from 300_000 to 90_000 with an inline comment documenting the RFC 2177 envelope. 22 test fixtures updated from `idleTimeout: 300_000`/`300000` → `90_000`/`90000`; only `test/unit/imap/client.test.ts:14` retains `300_000` with an explanatory comment block (those tests pin the old default to verify the pre-schema-change wedge path).
...
- moveMessage passes WRITE_TIMEOUT_MS, fetchAllMessages BULK_FETCH_TIMEOUT_MS, fetchNewMessages WRITE_TIMEOUT_MS, searchByHeader WRITE_TIMEOUT_MS, deleteMessage LIST_TIMEOUT_MS.
